...
|--------|----------|--------|----------|
| V1.0 | 2026-05-28 | — | 初稿 |
| V1.1 | 2026-06-01 | — | 备货减货不再产生 BOL「已作废」；BOL 自发车起派送生效；派送管理列表去掉已作废态 |
| V1.2 | 2026-06-01 | — | 员工端退仓弹窗按钮：通知仓库退仓 / 确认退仓完成（对应原退仓发起/退仓执行） |
| V1.3 | 2026-06-01 | — | 退仓待执行列表行操作：退仓完成、撤销退仓 |
| V1.4 | 2026-06-01 | — | 整车退仓新增库位号；确认退仓完成时必填 |
| V1.5 | 2026-06-01 | — | 整车/单项退仓新增实际到仓时间；确认退仓完成时必填 |
...
**列表与作业口径：** FBA 派送管理为**已发车后**作业列表，默认只展示**有效 BOL**（排除**已退仓**）；退仓待执行可展示以便对单。**已退仓**及减货剔除记录可在计划单/装车单详情与操作日志中查看。重新排货须使用**新 BOL 号**，不得复用**已退仓** BOL。
...
- **操作流程：**
  ...
  2. **单项/整车退仓：** 员工端弹窗底部二选一（**通知仓库退仓** / **确认退仓完成**，对应下述动作）：  
     - **退仓发起（通知仓库退仓）：** 向仓库下发指令；单项 → 目标 BOL **退仓待执行**；整车 → 装车单及该单下**全部 BOL** **退仓待执行**。  
...
| 单项退仓 | 员工端·**通知仓库退仓** | 员工端·**确认退仓完成** 或 PDA | 确认完成须**实际到仓时间**→**已退仓**；货件→**在库** |
| 整车退仓 | 员工端·**通知仓库退仓** | 员工端·**确认退仓完成** 或 PDA | 确认完成须**实际到仓时间**+**库位号**→全部 BOL **已装车**；不回在库 |
...
- FBA 派送管理列表 Tab（已装车/运输中/已签收/退仓相关等）语义为 **派送生效后的 BOL 层**；**不含「已作废」Tab**（备货减货不产生该态）。  
- 详情区分当前有效 BOL 与历史 BOL（**已退仓**等）；退仓待执行可对单展示；备货减货痕迹在计划/装车单日志中查看。  
